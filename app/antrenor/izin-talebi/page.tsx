...
import { CalendarDays, Send, Loader2, Clock, CheckCircle, XCircle, AlertCircle } from 'lucide-react'
...
export default function IzinTalebiPage() {
  ...
  useEffect(() => {
    fetch('/api/antrenor/izin-talebi')
      .then((r) => r.json())
  ...
  const handleGonder = async () => {
    if (!startDate || !reason.trim()) {
      alert('Tarih ve sebep zorunludur.')
      return
    }
    setSubmitting(true)
    setSuccess(false)
    try {
      const res = await fetch('/api/antrenor/izin-talebi', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ start_date: startDate, end_date: endDate || startDate, reason }),
      })
  ...
  return (
    <main className="p-4 space-y-4">
      <div>
        ...
        <p className="text-sm text-zinc-400">Tarih seçin, sebebinizi yazın ve gönderin.</p>
      </div>

      {/* Form */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 space-y-3">
        <h3 className="text-sm font-semibold text-white">Yeni İzin Talebi</h3>
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <label className="text-xs font-medium text-zinc-400">Başlangıç Tarihi</label>
            <input
              type="date"
              value={startDate}
          ...
          <div>
            <label className="text-xs font-medium text-zinc-400">Bitiş Tarihi (opsiyonel)</label>
            <input
              type="date"
              value={endDate}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
        ...
        {talepler.length === 0 ? (
          <div className="text-center py-4">
            ...
            <p className="text-xs text-zinc-500">Henüz izin talebi yok.</p>
          </div>
        ) : (
